...
- [ ] `src/services/backgroundRemovalService.ts`, `src/sw.ts`, `astro.config.ts` — Background-removal asset details are duplicated across runtime loading and caching code (hardcoded IMGLY CDN path/version and related cache assumptions). A package upgrade can easily desynchronize processing, service-worker caching, and offline support.
- [ ] `src/components/app/UploadArea.tsx`, `src/components/app/ProcessingControls.tsx`, `src/services/validationService.ts` — Supported-format knowledge is split across the file input `accept` string, format-select options, and validation logic. That duplication makes future format changes easy to implement inconsistently.
