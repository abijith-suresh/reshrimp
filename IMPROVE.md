...
- [x] `src/services/imageService.ts`, `src/utils/imageUtils.ts`, `src/types/image.ts`, `src/services/backgroundRemovalService.ts`, `src/test/mocks.ts` — Removed dead exports/types that had drifted from the current single-image product (`resizeImage`, `convertFormat`, `compressImage`, `getImageDimensions`, `clamp`, `ImageState`, `isBackgroundRemovalSupported`, `getLatestCanvasMock`). This keeps the public surface easier to understand.
