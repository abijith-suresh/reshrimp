...
describe("convertToPx", () => {
  ...
  it("px: returns the value rounded", () => {
    expect(convertToPx(1080, "px", originalPx)).toBe(1080);
    expect(convertToPx(1080.6, "px", originalPx)).toBe(1081);
  });

  it("%: converts percentage of original dimension", () => {
    expect(convertToPx(50, "%", 1920)).toBe(960);
    expect(convertToPx(100, "%", 1920)).toBe(1920);
    expect(convertToPx(25, "%", 1000)).toBe(250);
  });
});
...
describe("convertFromPx", () => {
  ...
  it("px: returns the value unchanged", () => {
    expect(convertFromPx(1080, "px", originalPx)).toBe(1080);
  });

  it("%: converts px back to percentage of original", () => {
    expect(convertFromPx(960, "%", 1920)).toBe(50);
    expect(convertFromPx(1920, "%", 1920)).toBe(100);
  });

  it("%: returns 0 when originalPx is 0", () => {
    expect(convertFromPx(100, "%", 0)).toBe(0);
  });

  it("round-trip px → unit → px stays within 1px", () => {
    const px = 1280;
    for (const unit of ["px", "%"] as const) {
      const display = convertFromPx(px, unit, originalPx);
      const backToPx = convertToPx(display, unit, originalPx);
